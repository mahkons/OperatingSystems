...
#define S(TN, PL) SETGATE(idt[TN], 0, GD_KT, HANDLER(TN), PL)
...
trap_init_percpu(void)
{
	...
	ltr(GD_TSS0 + NCPU * (thiscpu->cpu_id));
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if (UXSTACKTOP-PGSIZE <= tf->tf_esp && tf->tf_esp < UXSTACKTOP) {
        stack_addr = tf->tf_esp - 4; 
    } else {
        stack_addr = UXSTACKTOP; 
    }

    stack_addr -= sizeof(struct UTrapframe);
    struct UTrapframe* utrapframe = (struct UTrapframe*) stack_addr;
    ...
    curenv->env_tf.tf_esp =  stack_addr; 
